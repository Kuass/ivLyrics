...
const Utils = {
  ...
  async checkForUpdates() {
    try {
      ...
      const urls = [
        `https://raw.githubusercontent.com/${FORK_REPO}/${FORK_BRANCH}/version.txt`,
        `https://cdn.jsdelivr.net/gh/${FORK_REPO}@${FORK_BRANCH}/version.txt`,
      ];
      ...
      if (updateInfo.hasUpdate) {
        ...
        if (!isDismissed) {
          ...
          window.ivLyrics_updateInfo = {
            ...
            releaseUrl: `https://github.com/${FORK_REPO}/releases/tag/v${updateInfo.latestVersion}`,
          };
   ...
   */
  getInstallCommand() {
    // This fork is managed from the terminal by scripts/ivlyrics (macOS only).
    const commands = {
      windows: "# Windows is not supported by this fork",
      mac: "ivlyrics update",
      linux: "# Linux is not supported by this fork",
    };
   ...
   */
  openUpdaterProtocol() {
    // The fork is updated from the terminal (ivlyrics update); open the guide instead.
    const url = `https://github.com/${FORK_REPO}/tree/${FORK_BRANCH}#업데이트`;
  ...
  normalizeVideoSkipSegments(segments, maxSegments = 10) {
    ...
    const normalized = segments
      ...
      .map((segment) => {
        ...
        if (roundedStart < 0 || roundedEnd > 3600 || roundedEnd - roundedStart < 0.1) {
          return null;
        }
  ...
  mapVideoTimeWithSkipSegments(baseVideoTime, segments, anchorTime = 0) {
    let mappedTime = Number(baseVideoTime);
    if (!Number.isFinite(mappedTime)) return 0;
    const parsedAnchor = Number(anchorTime);
    const anchor = Number.isFinite(parsedAnchor) ? Math.max(0, parsedAnchor) : 0;

    for (const segment of this.normalizeVideoSkipSegments(segments)) {
      if (segment.end <= anchor) continue;
      const effectiveStart = Math.max(anchor, segment.start);
      if (mappedTime + 0.001 >= effectiveStart) {
        mappedTime += segment.end - effectiveStart;
      }
    }
    return mappedTime;
  },

  getVideoSkipSegmentAtTime(videoTime, segments, anchorTime = 0) {
    const parsedTime = Number(videoTime);
    if (!Number.isFinite(parsedTime)) return null;
    const parsedAnchor = Number(anchorTime);
    const anchor = Number.isFinite(parsedAnchor) ? Math.max(0, parsedAnchor) : 0;
    return this.normalizeVideoSkipSegments(segments).find((segment) =>
      segment.end > anchor &&
      parsedTime + 0.001 >= Math.max(anchor, segment.start) &&
      parsedTime < segment.end - 0.001
    ) || null;
  },
  ...
  /**
   * 좋아요가 싫어요 이상인 커뮤니티 영상 중 하나를 무작위로 선택합니다.
   * 후보가 여러 개면 같은 곡에서 직전에 선택한 영상은 한 번 피합니다.
   */
  async getRandomCommunityVideo(trackUri) {
    const data = await this.getCommunityVideos(trackUri);
    const videos = Array.isArray(data?.videos) ? data.videos : [];
    const seenVideoIds = new Set();
    const eligibleVideos = videos.filter((video) => {
      const videoId = typeof video?.youtubeVideoId === "string"
        ? video.youtubeVideoId.trim()
        : "";
    ...
    const trackKey = this.extractTrackId(trackUri) || trackUri;
    const previousVideoId = this._lastRandomCommunityVideos.get(trackKey);
    const selectionPool = eligibleVideos.length > 1
      ? eligibleVideos.filter((video) => video.youtubeVideoId !== previousVideoId)
      : eligibleVideos;
    const selectedVideo = selectionPool[
      Math.floor(Math.random() * selectionPool.length)
    ];

    this._lastRandomCommunityVideos.set(trackKey, selectedVideo.youtubeVideoId);

    return {
      youtubeVideoId: selectedVideo.youtubeVideoId,
      youtubeTitle: selectedVideo.youtubeTitle,
      captionStartTime: Number(selectedVideo.startTime) || 0,
      skipSegments: this.normalizeVideoSkipSegments(selectedVideo.skipSegments),
      communityEntryId: selectedVideo.id || null,
      isAutoGenerated:
        selectedVideo.submitterId === "system" ||
        selectedVideo.isAutoGenerated === true,
      isRandomCommunity: true,
    };
  },

  getCommunityVideoSelectionInfo(video) {
    if (!video?.youtubeVideoId) return null;
    return {
      youtubeVideoId: video.youtubeVideoId,
      youtubeTitle: video.youtubeTitle,
      captionStartTime: Number(video.startTime) || 0,
      skipSegments: this.normalizeVideoSkipSegments(video.skipSegments),
      communityEntryId: video.id || null,
      isAutoGenerated:
  ...
  /**
   * Revalidates a saved community selection against the current user's votes.
   * If the selected entry is now hidden by a dislike, returns the highest-ranked
   * remaining visible entry, or null when the video background must fall back.
   */
  async resolveHiddenCommunityVideo(trackUri, selectedVideo, skipCache = true) {
    if (!selectedVideo?.youtubeVideoId) {
      return { hidden: false, replacement: null };
    }

    const data = await this.getCommunityVideos(trackUri, skipCache);
    const videos = Array.isArray(data?.videos) ? data.videos : [];
    const selectedEntry = videos.find((video) =>
      (selectedVideo.communityEntryId && video.id === selectedVideo.communityEntryId) ||
      video.youtubeVideoId === selectedVideo.youtubeVideoId
    );

    if (selectedEntry?.userVote !== -1) {
      return { hidden: false, replacement: null };
    }

    const seenVideoIds = new Set([selectedVideo.youtubeVideoId]);
    const replacementEntry = videos.find((video) => {
      const videoId = typeof video?.youtubeVideoId === "string"
        ? video.youtubeVideoId.trim()
        : "";
    ...
    return {
      hidden: true,
      replacement: this.getCommunityVideoSelectionInfo(replacementEntry),
    };
  },
    ...
    try {
      const response = await fetch('https://lyrics.api.ivl.is/lyrics/youtube/community', {
        ...
        body: JSON.stringify({
          ...
          videoTitle,
          startTime,
          skipSegments: this.normalizeVideoSkipSegments(skipSegments)
        })
      });
      const data = await response.json();
  ...
  /**
   * 자막 본문에서 첫 큐의 시작 시각(초)을 읽는다. WebVTT, TTML, YouTube srv3/json3 형식을 받는다.
   * 첫 큐는 보통 첫 가사가 불리는 시점이라서 영상과 가사를 맞추는 기준점으로 쓴다.
   */
  parseFirstCaptionCueSeconds(body) {
    const text = String(body || "");
    if (!text) return null;
    const vtt = text.match(/(?:^|\n)\s*(?:(\d+):)?(\d{1,2}):(\d{2})[.,](\d{1,3})\s*-->/);
    if (vtt) {
      const hours = Number(vtt[1] || 0);
      const millis = Number(String(vtt[4]).padEnd(3, "0"));
      return hours * 3600 + Number(vtt[2]) * 60 + Number(vtt[3]) + millis / 1000;
    }
    const srv3 = text.match(/<p[^>]*\bt="(\d+)"/);
    if (srv3) return Number(srv3[1]) / 1000;
    const json3 = text.match(/"tStartMs"\s*:\s*(\d+)/);
    if (json3) return Number(json3[1]) / 1000;
    const ttml = text.match(/\bbegin="(?:(\d+):)?(\d{1,2}):(\d{2})(?:[.,](\d{1,3}))?"/)
      || text.match(/\bbegin="(\d+(?:\.\d+)?)s"/);
    if (ttml) {
      if (ttml.length === 2) return Number(ttml[1]);
      const millis = Number(String(ttml[4] || "0").padEnd(3, "0"));
      return Number(ttml[1] || 0) * 3600 + Number(ttml[2]) * 60 + Number(ttml[3]) + millis / 1000;
    }
    return null;
  },

  /**
   * 첫 가사와 영상 첫 자막의 간격이 뮤직비디오 인트로로 볼 수 있는 범위인지 확인한다.
   * 대사가 긴 단편 영화형 MV나 자막이 엉뚱한 영상은 기준점으로 쓰지 않는다.
   */
  isPlausibleCaptionAnchor(captionStartSec, lyricsStartSec = null) {
    if (!Number.isFinite(captionStartSec) || captionStartSec < 0 || captionStartSec > 240) return false;
    if (!Number.isFinite(lyricsStartSec)) return true;
    const offset = captionStartSec - lyricsStartSec;
    return offset >= -15 && offset <= 150;
  },

  /**
   * 공식 채널로 교체한 영상은 서버가 준 자막 시작 시각이 없다. Piped가 노출하는 YouTube 자막
   * (자동 생성 우선)에서 첫 큐를 읽어 가사와 영상을 맞추는 기준점으로 삼는다.
   */
  async resolveYouTubeCaptionStartTime(videoId, { lyricsStartSec = null, timeoutMs = 4000 } = {}) {
    if (!videoId) return null;
    const pickTrack = (subtitles) => {
      const list = Array.isArray(subtitles) ? subtitles.filter((track) => track?.url) : [];
      return list.find((track) => track.autoGenerated === true) || list[0] || null;
    };
    const attempts = this.PIPED_INSTANCES.map(async (host) => {
      const response = await this._fetchWithTimeout(`${host}/streams/${encodeURIComponent(videoId)}`, timeoutMs);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      const track = pickTrack(data?.subtitles);
      if (!track) throw new Error("no subtitles");
      const captionResponse = await this._fetchWithTimeout(track.url, timeoutMs);
      if (!captionResponse.ok) throw new Error(`HTTP ${captionResponse.status}`);
      const seconds = this.parseFirstCaptionCueSeconds(await captionResponse.text());
      if (!this.isPlausibleCaptionAnchor(seconds, lyricsStartSec)) throw new Error("implausible anchor");
      return seconds;
    });
    try {
  ...
  async searchYouTubeCandidates(query, timeoutMs = 4000) {
    const instances = this.PIPED_INSTANCES;
  ...
  async preferOfficialYouTubeVideo(videoInfo, meta = {}) {
    ...
    // 자막 시작 시각이 없는 영상은 가사와 영상을 맞출 기준점이 없어 인트로만큼 어긋난다.
    // 원래 영상을 유지하는 경우에도 기준점을 한 번 채워 둔다.
    const withCaptionAnchor = async (info) => {
      if (info.captionStartTime !== null && info.captionStartTime !== undefined) return info;
      const captionStartTime = await this.resolveYouTubeCaptionStartTime(info.youtubeVideoId, {
        lyricsStartSec: meta.lyricsStartSec ?? null,
      });
      if (captionStartTime === null) return info;
      return { ...info, captionStartTime, isAutoGenerated: true, captionAnchorSource: "youtube-captions" };
    };

    const owner = await this.getYouTubeVideoOwner(videoId);
    if (owner && this.isOfficialArtistChannel(owner.channel, artists)) {
      return withCaptionAnchor({ ...videoInfo, officialChecked: true, youtubeTitle: owner.title || videoInfo.youtubeTitle });
    }

    const query = `${artists[0]} ${trackName} official music video`;
    const candidates = await this.searchYouTubeCandidates(query);
    if (!candidates.length) return withCaptionAnchor({ ...videoInfo, officialChecked: true });
    ...
    if (!best || best.score < 120) {
      // 규칙이 확답하지 못한 후보는 AI 제공자에게 제목·채널·길이만 보내 판정을 받는다.
      const judged = await this.judgeOfficialVideoWithAI(scored, { trackName, artists, durationSec: meta.durationSec || 0 });
      if (judged) best = judged;
    }
    if (!best || best.candidate.videoId === videoId) {
      return withCaptionAnchor({ ...videoInfo, officialChecked: true });
    }

    window.__ivLyricsDebugLog?.(
      `[ivLyrics] Official video preferred: ${videoId} -> ${best.candidate.videoId} (${best.candidate.channel})`
    );
    return withCaptionAnchor({
      ...videoInfo,
      ...
      officialJudge: best.judge || "rules",
  ...
  /**
   * 규칙 점수가 애매한 후보(오디오 전용 제외, 상위 6개)를 AI에게 보내 공식 뮤직비디오를 고른다.
   * 설정이 꺼져 있거나 LLM 제공자가 없으면 null을 돌려준다.
   */
  async judgeOfficialVideoWithAI(scored, meta) {
    if (window.CONFIG?.visual?.["video-official-ai-judge"] === false) return null;
    const manager = window.AIAddonManager;
    if (typeof manager?.judgeOfficialMusicVideo !== "function" || !manager.hasJsonProvider?.()) return null;
    const pool = scored
      .filter((entry) => entry.score >= 40 && !this.isAudioOnlyUpload(entry.candidate))
      .slice(0, 6);
    if (!pool.length) return null;
    try {
      const videoId = await manager.judgeOfficialMusicVideo({
        ...meta,
        candidates: pool.map((entry) => entry.candidate),
      });
      const chosen = pool.find((entry) => entry.candidate.videoId === videoId);
      return chosen ? { ...chosen, judge: "ai" } : null;
    } catch (error) {
      console.warn("[ivLyrics] AI official video judge failed:", error?.message || error);
      return null;
    }
  },
