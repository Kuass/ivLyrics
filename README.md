...
- 제거한 기능이 남긴 CSS 규칙 619개(약 130KB)와 번역 문구 522개를 지웠습니다. 남은 규칙은 실제로 JS에서 쓰이는 클래스만 참조합니다.
...
단위 테스트는 `node --test tests/`로 실행합니다. 영상 선택 채점 로직은 `tests/official_video_preference.test.mjs`가 검증합니다.

저장소를 그대로 `~/.config/spicetify/CustomApps/ivLyrics`에 두지 않아도 됩니다. 수정 후에는 `ivlyrics update --ref <브랜치>`로 설치하거나, 로컬에서 `rsync`로 복사한 뒤 `spicetify apply`를 실행합니다.
