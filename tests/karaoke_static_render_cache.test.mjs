...
const FORK_INLINE_PRONUNCIATION_SLOT = 'lyrics-lyricsContainer-LyricsLine-phonetic lyrics-karaoke-part-subline';
// The fork appends an inline-pronunciation fallback slot to every karaoke line.
// Drop it so glyph output stays comparable with the upstream baseline. A value a
// replacer returns is serialized without being passed back in, so every branch
// that yields a karaoke line has to strip the slot itself.
const stripForkPronunciationSlot = node =>
  typeof node?.props?.className === 'string' && node.props.className.startsWith('lyrics-karaoke-line')
    ? { ...node, children: node.children.filter(child =>
      child !== null && child?.props?.className !== FORK_INLINE_PRONUNCIATION_SLOT) }
    : node;
const normalize = value => JSON.parse(JSON.stringify(value, (key, entry) => {
  ...
  if (entry?.props?.className === 'lyrics-vocal-main') return stripForkPronunciationSlot(entry.children[0]);
  if (stripForkPronunciationSlot(entry) !== entry) return stripForkPronunciationSlot(entry);
...
const createHarness = (source = currentSource, options = {}) => {
  ...
  const CONFIG = { visual: {
    ...
    // The fork renders inline pronunciation; keep it off so glyph output
    // stays comparable with the upstream baseline revision.
