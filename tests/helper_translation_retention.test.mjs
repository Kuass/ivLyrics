...
// Exercise the actual helper sender, preservation and mapper (the fork has no desktop overlay). Capture the complete
...
const load = () => {
	...
	const context = vm.createContext({
		...
		// The fork's helper sender is off unless the user enabled it.
		Spicetify: { Player: player, LocalStorage: { get: key => key === 'ivLyrics:visual:lyrics-helper-enabled' ? 'true' : null, set() {} } },
	...
	vm.runInContext([
		...
		section('    const normalizeSenderProgressTiming =', '    window.LyricsService = LyricsService;'),
		'globalThis.senders = [lyricsHelperSender];',
		'globalThis.getKey = getOverlayPresentationKey;',
	].join('\n'), context);
	const [helper] = context.senders;
	window.lyricsHelperSender = helper;
	for (const [name, sender] of [['helper', helper]]) {
	...
	return {
		helper, packets, window, player, getKey: context.getKey,
...
test('lyrics-ready listeners retain the producer pronunciation notation and matching translation', async () => {
	const h = load();
	await h.send(h.helper, richLyrics(), richContext());
...
test('legacy lyrics-ready events without mode fields display their supplied supplements', async () => {
	...
	await flush();
	for (const name of ['helper']) assertSupplements(h.last(name));
});
...
test('changed vocal text or timing does not inherit another vocal translation', async () => {
	for (const changed of ['text', 'timing']) {
		const h = load();
		await h.send(h.helper, vocalLyrics(true), richContext());
		const next = vocalLyrics();
		if (changed === 'text') next[0].vocals.background[0].text = 'Different';
		else next[0].vocals.background[0].syllables[0].startTime += 1000;
		await h.send(h.helper, next, richContext());
		assert.equal(h.last('helper')[0].vocals.background[0].translation, '');
		assert.equal(h.last('helper')[0].vocals.background[0].phonetic, '');
	}
});
...
test('an explicit mode change hides every vocal supplement without mutating their original data', async () => {
	...
	await h.send(h.helper, lines, richContext());
	await h.send(h.helper, lines, { ...richContext(), displayMode1: 'none', displayMode2: 'none' });
	const vocals = h.last('helper')[0].vocals;
...
test('a new track cannot inherit translation even if its text and timing are identical', async () => {
	...
	await h.send(h.helper, rawLyrics(), null, 'normal', h.player.data.item.uri);
	assertSupplements(h.last('helper'), [null, null], [null, null]);
});
