...
// Karaoke rows carry their reading inline in the fork, so the standalone
// phonetic supplement is suppressed there. Align the baseline guard with it.
...
function renderer(source) {
  ...
  const context = vm.createContext({
    CONFIG: { visual: { 'inactive-color': '#999', 'pronunciation-inline': false } },
...
test('row movement reuses text and supplements while preserving baseline DOM, copy and accessibility props', () => {
  for (const isKara of [false, true]) {
    ...
    const props = { line, mainText: line.text, subText: 'nihongo', subText2: '일본어',
      isKara, seekTime: 1500, culturalNote: [{ marker: 1, expression: '日本語', note: 'Japanese' }] };
    let last;
