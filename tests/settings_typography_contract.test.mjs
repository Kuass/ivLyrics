...
// The right-margin slider was intentionally retired, and the fork adds its own
// inline-pronunciation toggle. Keep the rest of the released tree as the oracle.
const ignoredItemKeys = new Set([retiredFullscreenMarginKey, "pronunciation-inline"]);
...
// The fork's header and injected CSS differ from the release, so compare the
// typography controls themselves rather than the whole tab tree.
const typographyItems = (tree) => typographyControls(tree).map((control) => normalize(control.item));

// The fork has no LP-mode typography, so only the appearance tab keeps the released oracle.
for (const tab of ["appearance"]) {
  test(`${tab} typography retains the released tree, defaults, and language updates`, () => {
    for (const value of [undefined, null, false, 0, "", 27, "Roboto"]) {
      ...
      assert.deepEqual(typographyItems(first), typographyItems(before.render(tab)));
      ...
      assert.deepEqual(typographyItems(after.render(tab)), typographyItems(before.render(tab)));
    }
  });
